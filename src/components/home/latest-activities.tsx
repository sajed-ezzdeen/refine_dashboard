...
const LatestActivities = () => {  
    ...
    const { data: deals, isLoading: isLoadingDeals } = useList({ 
        resource: 'deals',  
        queryOptions: { enabled: !!dealIds?.length }, // Enable the query only if dealIds has a length  
        pagination: {
            mode: 'off',     
        }, 
  ...
  return (
    ...
    > 
        {isLoading ? (
            ...
            dataSource={Array.from({ length: 5}).map((_, i) => ({id: i}))} // Create an array of 5 items
            renderItem={(_, index) => ( 
